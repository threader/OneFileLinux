...
Runs on any UEFI computer (PC or Mac) with 'some' effort. Since this project has diverged a bit from the original OFL, like building with Buildroot and using the -march/mcpu/mtune compiler flags set to 'native', both for gcc and llvm/clang (see buildroot/packages/Makefile.in), so the compiler uses the full instructionset avaialble for the architecture of the machine it's building on, and thereby will boot only/is only meanto to boot on that same computer, this also goes for the Linux kernel configuration, as 'make localyesconfig' is used, the rootfs.cpio.xz (351mb) is appended to the bzimage(690mb), the kernel commandline to boot is 'root=/dev/ram0 init=/sbin/init'. The kernel .config is expecting 'you' to tweak it to include the proper drivers for display etc. It will boot in QEMU/KVM and In my testing it passes secure boot as built on Debian Stable distro. I've not quite landed on the BuildRoot .config.
